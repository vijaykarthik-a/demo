        ...
        const SUPA_URL_HARDCODED = 'https://dwyvtklbynsyrzjhvmtf.supabase.co';
        const SUPA_KEY_HARDCODED = 'YOUR_ANON_KEY_HERE'; // ⚠️ Go to Supabase → Settings → API → copy "anon public" key (eyJ...) and paste here
        ...
        let supaClient = null;
        if (appState.supaUrl && appState.supaKey && appState.supaKey !== 'YOUR_ANON_KEY_HERE') {
        ...
        function openSyncModal() { showModal('sheet-sync'); }
        function saveSyncSettings() {
            appState.supaUrl = document.getElementById('supa-url').value.trim();
            appState.supaKey = document.getElementById('supa-key').value.trim();
            localStorage.setItem('df_supaUrl', appState.supaUrl);
            localStorage.setItem('df_supaKey', appState.supaKey);
        ...
        // ✅ Google login removed — GitHub only
        async function signInWithGitHub() {
            if (supaClient) {
                const { data, error } = await supaClient.auth.signInWithOAuth({ provider: 'github', options: { redirectTo: window.location.origin } });
                if (error) return showError(error.message);
            } else {
